...
print(f"Loaded {len(captures)} frames for TSDF fusion with per-frame 3D crop")
...
fx, fy, cx, cy = INTRINSICS.get_focal_length()[0], INTRINSICS.get_focal_length()[1], \
                 INTRINSICS.get_principal_point()[0], INTRINSICS.get_principal_point()[1]
...
for i, cap in enumerate(captures):
    ...
    # project depth points to base frame
    h, w = depth_np.shape
    ys, xs = np.meshgrid(np.arange(h), np.arange(w), indexing="ij")
    z = depth_np / 1000.0  # to meters
    valid = z > 0
    x = (xs - cx) * z / fx
    y = (ys - cy) * z / fy
    ...
    if (i + 1) % 3 == 0:
        print(f"Integrated {i+1}/{len(captures)} frames")

pcd = tsdf.extract_point_cloud()
pcd, _ = pcd.remove_statistical_outlier(nb_neighbors=30, std_ratio=2.0)
print(f"Extracted {len(pcd.points)} points after cropping + fusion")
