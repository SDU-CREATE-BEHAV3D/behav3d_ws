...
`color_to_depth` writes output into `alignment_test/` under the session root. The TSDF scripts read from that folder.
...
- `alignment_test/` under the session root
...
- `tsdf_surface_rgb_colored.ply`
- `tsdf_surface_confidence_colored.ply` (if enabled)
...
- `tsdf_cropped` and `tsdf_object_extract` read aligned color images from `alignment_test/`. Run `color_to_depth` first.
- `tsdf_object_extract` expects `table_plane.json` to exist in the session root. Generate it with `tsdf_cropped` in plane fit mode.
