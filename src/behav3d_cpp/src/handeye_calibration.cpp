  ...
  for (const auto &entry : root["captures"])
  {
                                               ...
                                               const std::string &method_name)
    {
      try
      {
        ...
        HE_INFO(this, "calibration solved (method=%s).", method_name.c_str());
        return true;
      }
      catch (const cv::Exception &e)
      {
        HE_ERROR(this, "OpenCV calibrateHandEye failed (method=%s): %s",
                 method_name.c_str(), e.what());
        return false;
      }
      catch (const std::exception &e)
      {
        HE_ERROR(this, "calibrateHandEye threw std::exception (method=%s): %s",
                 method_name.c_str(), e.what());
        return false;
      }
    }
