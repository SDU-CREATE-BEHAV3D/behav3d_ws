...
def test_build_evaluation_scene_uses_dds_geometry_and_state_colors() -> None:
    ...
    scene = build_evaluation_scene(
        ...
        scan_faces=faces,
        phi_contour_points=np.array([[0.5, 0.0, 1.0], [0.5, 1.0, 1.0]]),
        phi_contour_lines=np.array([[0, 1]], dtype=np.int32),
        evaluation=evaluation,
    )

    colors = scene.goal_mesh.vertex_colors
    assert colors is not None
    assert np.array_equal(colors[0], FILLED_COLOR)
    ...
    assert scene.phi_contour_points.shape == (2, 3)
    assert scene.phi_contour_lines.tolist() == [[0, 1]]
